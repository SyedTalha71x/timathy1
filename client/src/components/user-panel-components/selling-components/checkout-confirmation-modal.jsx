...
const CheckoutConfirmationModal = ({
  ...
  return (
    <div className="fixed inset-0 cursor-pointer open_sans_font w-full h-full bg-black/70 flex items-center justify-center z-[1000] p-4">
      <div className="bg-[#181818] rounded-xl w-full max-w-lg my-8 relative max-h-[90vh] flex flex-col">
        {/* Header */}
        ...
        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-5">
          ...
          <div className="bg-[#101010] rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-gray-400 text-xs mb-1">Member</p>
                ...
                {member?.type && !sellWithoutMember && (
                  <p className="text-gray-500 text-xs mt-0.5">{member.type}</p>
                )}
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {products.map((item) => (
                  <div
                    key={item.id}
                    className="bg-[#101010] rounded-xl p-3 flex justify-between items-center"
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0">
                        {item.image ? (
                          ...
                          <div className="w-full h-full bg-orange-500 flex items-center justify-center text-white text-[8px] font-medium text-center p-1">
          ...
          <div className="bg-[#101010] rounded-xl p-4 space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white">{formatCurrency(vatBreakdown.totalNet)}</span>
            </div>
            
            {vatBreakdown.totalVat19 > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">VAT 19%</span>
                <span className="text-white">{formatCurrency(vatBreakdown.totalVat19)}</span>
              </div>
            )}
            ...
            {vatBreakdown.totalVat7 > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white">{formatCurrency(vatBreakdown.totalVat7)}</span>
              </div>
            )}
        ...
        {/* Footer */}
        <div className="p-6 pt-4 border-t border-[#333333] flex-shrink-0">
          <div className="flex flex-row-reverse gap-3">
            <button
              onClick={onConfirm}
              className="flex-1 sm:flex-none sm:w-auto px-8 py-2.5 bg-orange-500 hover:bg-orange-600 text-sm text-white rounded-xl transition-colors font-medium"
            >
              Confirm Purchase
            </button>
            <button
