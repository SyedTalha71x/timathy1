...
const SendEmailModal = ({ sale, onClose }) => {
    const [emailData, setEmailData] = useState({
        ...
        body: `<p>Dear ${sale.member},</p><p><br></p><p>Please find attached your invoice for the recent purchase.</p><p><br></p><p><strong>Invoice Details:</strong></p><ul><li>Invoice #: ${sale.id}</li><li>Date: ${sale.date}</li><li>Total Amount: $${sale.totalAmount.toFixed(2)}</li></ul><p><br></p><p>Thank you for your business!</p><p><br></p><p>Best regards,<br>Your Business Team</p>`,
    })
    const [attachments, setAttachments] = useState([])

    // Generate PDF invoice content (you can replace this with actual PDF generation)
...
Thank you for your business!
    ...
    const handleSendEmail = async () => {
        ...
        try {
            ...
            // If no attachments, add the invoice automatically
            if (attachments.length === 0) {
                const invoiceContent = generateInvoicePDF()
                const blob = new Blob([invoiceContent], { type: 'text/plain' })
                const invoiceFile = new File([blob], `invoice-${sale.id}.txt`, { type: 'text/plain' })
                formData.append("attachments", invoiceFile)
            }
                            ...
                            <div className="bg-[#222222] border border-gray-600 rounded-lg p-4">
                                {attachments.length === 0 ? (
                                    <p className="text-gray-400 text-sm">
                                        No attachments added. The invoice will be automatically attached when sending.
                                    </p>
                                ) : (
