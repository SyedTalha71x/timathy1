...
const MemberOverviewModal = ({
  ...
  setSelectedMember,
  // New handlers for additional buttons (to be implemented)
        ...
        <div className="p-6">
          ...
          <div className="bg-[#161616] rounded-xl p-6">
              ...
              <div className="flex flex-col items-center">
                <div className="flex flex-col sm:flex-row items-center gap-2">
                  <h3 className="text-white font-medium text-lg">
                    ...
                    {selectedMember.dateOfBirth && ` (${calculateAge(selectedMember.dateOfBirth)})`}
                  </h3>
                  ...
                  <div className="flex items-center gap-2">
                    {selectedMember.isArchived ? (
                      ...
                      >
                        {selectedMember.isActive
                          ? "Active"
                          : `Paused${selectedMember.reason ? ` (${selectedMember.reason})` : ""}`}
                      </span>
                    )}

                    {isBirthday && isBirthday(selectedMember.dateOfBirth) && (
                ...
                <div className="text-sm mt-1 flex items-center gap-1">
                  ...
                  <span className="text-gray-400">
                    {selectedMember.memberType === "full" ? "Full Member" : "Temporary Member"}
                  </span>
                </div>
                ...
                <p className="text-gray-400 text-sm mt-1 text-center flex items-center">
                  {selectedMember.memberType === "full" ? (
                    ...
                    <>
                      No Contract - Auto-archive: {selectedMember.autoArchiveDate}
                      {selectedMember.autoArchiveDate && new Date(selectedMember.autoArchiveDate) <= new Date() && (
                ...
                {handleRelationFromOverview && (
                  <div className="mt-2">
                    ...
                    >
                      <Users size={12} />
                      Relations ({memberRelations ? Object.values(memberRelations[selectedMember.id] || {}).flat().length : 0})
                    </button>
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-5 gap-2 mt-6">
              ...
              <button
                onClick={handleTrainingPlansFromOverview || (() => console.log('Training Plans - To be implemented'))}
                className="text-white bg-black rounded-xl border border-slate-600 py-2 px-1 hover:border-slate-400 transition-colors text-sm flex items-center justify-center"
              ...
              <button
                onClick={handleDocumentFromOverview || (() => console.log('Documents - To be implemented'))}
                className="text-white bg-black rounded-xl border border-slate-600 py-2 px-1 hover:border-slate-400 transition-colors text-sm flex items-center justify-center"
