          ...
          <div className="mb-4 md:mb-6">
            ...
            <div className="space-y-3 md:space-y-4 max-h-[60vh] md:max-h-none overflow-y-auto px-1">
              {activityLogs.map((activity) => (
                <div key={activity.id} className="pl-3 md:pl-4 py-2 md:py-3 bg-[#222222] rounded-r-lg mx-1 md:mx-0">
                  <div className="flex items-start gap-2 md:gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-1 xs:gap-2">
                        <h4 className="font-semibold text-white text-sm md:text-base truncate">{activity.action}</h4>
                        <span className="text-xs text-zinc-400 bg-[#2a2a2a] px-2 py-1 rounded flex-shrink-0">{activity.timestamp}</span>
                      </div>
                      <p className="text-xs md:text-sm text-zinc-300 mt-1 break-words">{activity.description}</p>
                    </div>
                  </div>
                </div>
              ))}
