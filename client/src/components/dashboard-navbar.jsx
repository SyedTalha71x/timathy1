...
const Navbar = ({ onToggleSidebar, isSidebarCollapsed }) => {
  ...
  const languages = [
    ...
    },
    {
      code: "it",
      name: "Italian",
      flag: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/03/Flag_of_Italy.svg/1280px-flag_of_Italy.svg.png",
    },
  ]

  const navigate = useNavigate()

  const toggleNavbarDropdown = () => setIsNavbarDropdownOpen(!isNavbarDropdownOpen)
  ...
  const Modal = ({ isOpen, onClose, title, children }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[9999999999999] p-4">
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 w-full bg-[#1f1e1e] p-2 transition-all duration-500 flex items-center justify-between z-40 ${
      ...
      >
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 rounded-lg text-white hover:bg-zinc-700 lg:hidden"
        ...
        <div className="flex gap-2 items-center">
          {/* Studio Name and User Name - Hidden on small screens, visible on medium and up */}
          <div className="hidden md:flex bg-black text-sm rounded-md p-2 text-white items-center gap-2">
            <div className="font-bold">Studio One</div>
            <div>
              <Building2 size={16} />
            </div>
          </div>
          <div className="hidden md:block">
            <span className="font-bold text-md text-white">Samantha</span>
          </div>

          <div className="relative">
            <div onClick={toggleNavbarDropdown} className="flex items-center gap-1 cursor-pointer">
              <img src="/gray-avatar-fotor-20250912192528.png" alt="Profile" className="w-9 h-9 rounded-lg" />
            </div>
            {isNavbarDropdownOpen && (
              <div className="absolute right-0 top-10 w-48 bg-[#222222]/50 backdrop-blur-3xl rounded-lg shadow-lg z-50">
                <div className="py-2" role="menu">
                  {/* Studio and User Info - Only visible on small screens */}
                  <div className="md:hidden px-4 py-2 border-b border-zinc-600 mb-2">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="bg-black text-xs rounded-md p-1 px-2 text-white flex items-center gap-1">
                        <Building2 size={12} />
                        <span>Studio One</span>
                      </div>
                    </div>
                    <div className="text-sm font-semibold text-white">Samantha</div>
                  </div>
          ...
          <div className="relative">
            <button
              ...
              className="p-2 px-3 rounded-xl text-gray-500 bg-[#1C1C1C] cursor-pointer flex items-center gap-1 hover:bg-zinc-800 transition-colors"
              aria-label="Language Selection"
            >
              <Globe size={20} />
            </button>
            {isLanguageDropdownOpen && (
              <div className="absolute right-4 top-10 w-40 bg-[#222222]/50 backdrop-blur-3xl rounded-lg shadow-lg z-50">
                <div className="py-2" role="menu">
