    ...
    <div className="fixed inset-0 bg-black/50 flex p-2 justify-center items-center z-50 overflow-y-auto">
      <div className="bg-[#1C1C1C] p-6 rounded-xl w-full max-w-4xl my-8">
        ...
        <div className="flex border-b border-gray-700 mb-6">
          ...
          <button
            onClick={() => setActiveTab("specialNote")}
            className={`px-4 py-2 text-sm font-medium ${
              activeTab === "specialNote"
          ...
          <div className="space-y-4 text-white">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p>Lead Source</p>
                <p
                  className={`px-2 py-1 text-xs w-38 text-center mt-1 rounded-full ${getSourceColor(leadData.source)}`}
                >
                  {leadData.source || "N/A"}
                </p>
              </div>
        ...
        {activeTab === "specialNote" && (
          <div className="space-y-4 text-white relative z-10">
            <div className="bg-[#161616] rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Special Note</h3>
              {leadData.specialNote && leadData.specialNote.text ? (
                <div className="space-y-4">
                  <div className="bg-[#2F2F2F] rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm text-gray-400">Note Content</span>
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${
                          leadData.specialNote.isImportant ? "bg-red-900 text-red-300" : "bg-gray-900 text-gray-300"
                        }`}
                      >
                        {leadData.specialNote.isImportant ? "Important" : "Normal"}
                      </span>
                    </div>
                    <p className="text-white leading-relaxed">{leadData.specialNote.text}</p>
                  </div>

                  {(leadData.specialNote.startDate || leadData.specialNote.endDate) && (
                    <div className="bg-[#2F2F2F] rounded-lg p-4">
                      <h4 className="text-sm text-gray-400 mb-2">Validity Period</h4>
                      <div className="grid grid-cols-2 gap-4">
                        {leadData.specialNote.startDate && (
                          <div>
                            <span className="text-xs text-gray-500">Start Date</span>
                            <p className="text-white">{leadData.specialNote.startDate}</p>
                          </div>
                        )}
                        {leadData.specialNote.endDate && (
                          <div>
                            <span className="text-xs text-gray-500">End Date</span>
                            <p className="text-white">{leadData.specialNote.endDate}</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500">No special note available for this lead.</p>
                </div>
              )}
            </div>
          </div>
