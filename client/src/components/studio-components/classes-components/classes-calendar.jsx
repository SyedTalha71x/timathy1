...
const ClassesCalendar = forwardRef(({
  ...
  const formatDateRange = useCallback(() => {
    ...
    } else if (vt === "timeGridDay") { return new Date(view.currentStart).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}); }
    else if (vt === "dayGridMonth") { return new Date(view.currentStart).toLocaleDateString("en-US",{month:"long",year:"numeric"}); }
    return currentDateDisplay;
  }, [currentDateDisplay]);

  const broadcast = useCallback(() => { const d=formatDateRange(); setCurrentDateDisplay(d); onDateDisplayChange?.(d); }, [formatDateRange, onDateDisplayChange]);
  ...
  useEffect(()=>{const c=()=>{if(window.innerWidth<480&&calendarRef.current){const a=calendarRef.current.getApi();if(a.view.type!=="timeGridDay"){setCurrentViewType("timeGridDay");a.changeView("timeGridDay")}}};c();window.addEventListener("resize",c);return()=>window.removeEventListener("resize",c)},[]);
  useEffect(()=>{if(selectedDate&&calendarRef.current){const a=calendarRef.current.getApi(),vs=new Date(a.view.currentStart),ve=new Date(a.view.currentEnd),sel=new Date(selectedDate);if(sel<vs||sel>=ve)a.gotoDate(selectedDate)}},[selectedDate]);
  useEffect(()=>{let ro;if(calendarRef.current){const el=calendarRef.current.elRef.current;ro=new ResizeObserver(()=>{setTimeout(()=>calendarRef.current?.getApi()?.updateSize(),100)});if(el)ro.observe(el)}return()=>ro?.disconnect()},[]);

  // Tooltip
  const showTooltipHandler = (event, mouseEvent, el) => {
    clearTimeout(hideRef.current); clearTimeout(showRef.current);
    const cls = event.extendedProps?.classData; if (!cls) return;
    const target = el || mouseEvent?.target?.closest(".fc-event"); if (!target) return;
    showRef.current = setTimeout(() => {
      const r = target.getBoundingClientRect();
      setTooltip({ show:true, x:r.left+r.width/2, y:r.top-4,
        content:{ name:cls.typeName, trainer:cls.trainerName||"N/A", room:cls.room||"N/A",
          time:`${cls.startTime} - ${cls.endTime}`, enrolled:cls.enrolledMembers?.length||0,
          max:cls.maxParticipants||0, isFull:(cls.enrolledMembers?.length||0)>=(cls.maxParticipants||0),
          isCancelled:cls.isCancelled||false }
      });
    }, 150);
  };
  const hideTooltip = (now) => { clearTimeout(hideRef.current);clearTimeout(showRef.current); const h=()=>setTooltip({show:false,x:0,y:0,content:null}); now?h():(hideRef.current=setTimeout(h,50)); };
  useEffect(()=>()=>{clearTimeout(hideRef.current);clearTimeout(showRef.current)},[]);

  // Filter & map
  const filteredClasses = classesData.filter((c) => classFilters[c.typeId] !== false);
  const calendarEvents = filteredClasses.map((cls) => {
    if (!cls.date||!cls.startTime||!cls.endTime) return null;
    const ds = typeof cls.date==='string'?cls.date:formatDateLocal(cls.date);
    const enrolled = cls.enrolledMembers?.length||0;
    const isCancelled = cls.isCancelled||false;
    const isPast = cls.isPast||false;
    let bg = cls.color||"#6c5ce7", bc = bg;
    if (isCancelled) { bg="#6B7280"; bc="#6B7280"; }
    return { id:cls.id, title:cls.typeName, start:`${ds}T${cls.startTime}`, end:`${ds}T${cls.endTime}`,
      backgroundColor:bg, borderColor:bc, textColor:"#FFFFFF",
      extendedProps:{ classData:cls, isFull:enrolled>=(cls.maxParticipants||0), enrolled, maxParticipants:cls.maxParticipants, isCancelled, isPast }
    };
  }).filter(Boolean);

  const handleEventClick = (info) => { hideTooltip(true); const c=classesData.find(x=>x.id===Number(info.event.id)); if(c)onClassClick?.(c); };

  const handleDateSelect = (selectInfo) => {
    const clicked = selectInfo.jsEvent?.target;
    if (clicked) { if (clicked.classList?.contains('month-cls-tile')||clicked.closest('.month-cls-tile')) return; if (clicked.classList?.contains('month-more-link')||clicked.closest('.month-more-link')) return; }
    const vt = calendarRef.current?.getApi()?.view?.type;
    if (vt==="dayGridMonth") { const s=new Date(selectInfo.start);s.setHours(9,0,0,0); onDateSelect?.({start:s,end:new Date(s.getTime()+3600000),formattedTime:"09:00"}); }
    else { const s=selectInfo.start; onDateSelect?.({start:s,end:selectInfo.end,formattedTime:`${String(s.getHours()).padStart(2,'0')}:${String(s.getMinutes()).padStart(2,'0')}`}); }
  };
  ...
  return (
    <>
      <style>{`
        ...
        /* Cancelled - diagonal stripes (matches appointments) */
        ...
        /* Past - faded (matches appointments) */
      ...
      {tooltip.show && tooltip.content && (
        <div className="fixed z-[9999] pointer-events-none" style={{left:tooltip.x,top:tooltip.y,transform:"translate(-50%,-100%)"}}>
          <div className="bg-surface-dark text-content-primary p-3 rounded-lg shadow-lg border border-border max-w-xs">
            <div className="text-sm font-semibold mb-1">{tooltip.content.name} {tooltip.content.isCancelled && <span className="text-red-400 text-xs font-normal">(Cancelled)</span>}</div>
            ...
            <div className={`text-xs font-medium flex items-center gap-1 text-primary`}><ParticipantsIcon size={11}/>{tooltip.content.enrolled}/{tooltip.content.max}</div>
          </div>
          <div className="w-full flex justify-center -mt-[1px]"><div className="w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[8px]" style={{borderTopColor:'var(--color-surface-dark)'}}/></div>
        </div>
      )}
        ...
        <div className="w-full bg-surface-base flex-1 flex flex-col min-h-0">
          <div className="flex-1 min-h-0 overflow-auto pt-0">
            <FullCalendar
              ...
              height="auto" contentHeight="auto" locale="en"
              selectable headerToolbar={false} editable={false}
              ...
              eventOverlap={true} eventMinHeight={28} dayMaxEvents={false} eventMaxStack={10}
                      ...
                      {show.map((cls)=>{
                        ...
                        return(
                          <div key={cls.id} className="month-cls-tile" style={{backgroundColor:bg,marginBottom:'1px',padding:'3px 6px',borderRadius:'3px',height:'20px',width:'100%',overflow:'hidden',color:'#fff',cursor:'pointer',display:'flex',alignItems:'center',gap:'4px',fontSize:'10px',opacity,position:'relative'}}
                            ...
                            {cancelled&&<div style={{position:'absolute',inset:0,background:'repeating-linear-gradient(135deg,transparent,transparent 3px,rgba(255,255,255,.15) 3px,rgba(255,255,255,.15) 6px)',borderRadius:'inherit',pointerEvents:'none'}}/>}
                            <span style={{fontWeight:600,flex:1,overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis'}}>{cls.typeName}</span>
                            <span style={{display:'flex',alignItems:'center',gap:'2px',flexShrink:0,color:'#fff',opacity:.85}}><ParticipantsIcon/>{en}/{cls.maxParticipants}</span>
                          </div>
                        );
                      })}
                      {more>0&&<div className="month-more-link" style={{fontSize:'9px',color:'var(--color-content-muted)',padding:'1px 4px'}}>+{more} more</div>}
                    </div>
                  </div>
                );
              }}
