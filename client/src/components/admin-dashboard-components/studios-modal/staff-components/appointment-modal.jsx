...
const AppointmentModalStaff = ({
  isOpen,
  onClose,
  selectedStaff,
  getStaffAppointmentsStaff,
  appointmentTypesStaff,
  handleEditAppointmentStaff,
  handleDeleteAppointmentStaff,
  staffContingent,
  currentBillingPeriodStaff,
  handleManageContingentStaff,
  handleCreateNewAppointmentStaff
}) => {
        ...
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-medium text-white">
              {selectedStaff.title}'s Appointments
            </h2>
            <button
              onClick={onClose}
          ...
          <div className="space-y-3 mb-4">
            ...
            {getStaffAppointmentsStaff(selectedStaff.id).length > 0 ? (
              getStaffAppointmentsStaff(selectedStaff.id).map((appointment) => {
                const appointmentType = appointmentTypesStaff.find((type) => type.name === appointment.type);
                ...
                return (
                  <div
                    ...
                    onClick={() => handleEditAppointmentStaff(appointment)}
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleEditAppointmentStaff(appointment);
                          }}
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDeleteAppointmentStaff(appointment.id);
                          }}
                          className="p-1.5 bg-[#2F2F2F] text-white hover:bg-[#3F3F3F] rounded-full"
                        >
                          <Trash2 size={14} />
          ...
          <div className="flex items-center justify-between py-3 px-2 border-t border-gray-700 mb-4">
            <div className="text-sm text-gray-300">
              Contingent ({currentBillingPeriodStaff}):{" "}
              {staffContingent[selectedStaff.id]?.current?.used || 0} /{" "}
              {staffContingent[selectedStaff.id]?.current?.total || 0}
            </div>
            <button
              onClick={() => handleManageContingentStaff(selectedStaff.id)}
          ...
          <button
            onClick={handleCreateNewAppointmentStaff}
...
export default AppointmentModalStaff;