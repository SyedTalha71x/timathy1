...
export default function BroadcastModal({
  ...
  const handleBroadcastClick = () => {
    ...
    const forceEmailSettings = {
      ...settings,
      broadcastEmail: true,
      broadcastChat: false,
    }

    if (!emailOnly && activeMainTab === "push") {
      if (!selectedPushMessage) {
        alert("Please select a push message template")
        return
      }
      onBroadcast({
        message: selectedPushMessage,
        recipients,
        settings: { ...settings, broadcastEmail: false, broadcastChat: true },
      })
      onClose()
      return
    }

    // Email branch (emailOnly or email tab)
    let emailSubject = ""
    let emailBody = ""
    ...
    onBroadcast({ message: emailMessageObj, recipients, settings: forceEmailSettings })
    onClose()
  }
