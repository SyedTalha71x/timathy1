...
const PaymentRunsModal = ({ isOpen, onClose, paymentRuns = [], onDeletePaymentRun }) => {
  ...
  // IMPORTANT: Helper to get IBAN
  const getIban = (tx) => {
    return tx.iban || tx.memberIban || tx.bankIban || "DE89370400440532013000"
  }

  // Helper to get account holder (admin uses studioOwner)
...
${run.transactions.map((tx, idx) => {
  ...
  const accountHolder = escapeXml(getAccountHolder(tx));
  const iban = getIban(tx);
  const remittanceInfo = escapeXml(tx.services.map(s => s.name).join(', ')).substring(0, 140);
  ...
  // Generate and download PDF report using jsPDF - with Studio Name column
  const downloadPdfReport = (run) => {
    ...
    // ========== PAGE 2: Transaction Details (with Studio Name column) ==========
    ...
    const tableData = run.transactions.map((tx, idx) => [
      tx.studioNumber || tx.memberNumber || `S-${String(idx + 1).padStart(5, '0')}`,
      getStudioName(tx),
      getAccountHolder(tx),
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        2: { cellWidth: 38 },
        3: { cellWidth: 36 },
        4: { cellWidth: 48, font: 'courier', fontSize: 6.5 },
        5: { cellWidth: 50 },
              ...
              <div className="space-y-3">
                {paymentRuns.map((run, index) => (
                  <div key={run.id} className="bg-[#141414] rounded-xl overflow-hidden border border-gray-800 hover:border-gray-700 transition-colors">
                    ...
                    {/* Expanded Details - with Studio Name column */}
                    {expandedRun === run.id && (
                              ...
                              <tbody>
                                {run.transactions.map((tx, idx) => (
                                  <tr key={tx.id || idx} className="border-t border-gray-800">
                                    <td className="py-2 px-2 text-gray-300">{tx.studioNumber || tx.memberNumber || `S-${String(idx + 1).padStart(5, '0')}`}</td>
                                    <td className="py-2 px-2 text-white">{getStudioName(tx)}</td>
                                    <td className="py-2 px-2 text-white">{getAccountHolder(tx)}</td>
