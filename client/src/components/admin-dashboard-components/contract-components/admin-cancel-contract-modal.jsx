...
export function AdminCancelContractModal({ contract, onClose, onSubmit }) {
  ...
  const [cancelData, setCancelData] = useState({
    ...
    cancellationType: "extraordinary", // "extraordinary" | "throughAdmin"
  ...
  const cancellationTypeOptions = [
    ...
    { id: "throughAdmin", label: "Cancellation through Admin" },
  ]
          ...
          <div className={`bg-surface-dark/60 p-4 rounded-xl border border-border ${cancelData.cancellationType === 'throughAdmin' ? 'opacity-60' : ''}`}>
            <label className={`flex items-center space-x-2 ${cancelData.cancellationType === 'throughAdmin' ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
              <input
                ...
                onChange={(e) => {
                  if (cancelData.cancellationType !== 'throughAdmin') {
                    setCancelData({ ...cancelData, notificationRule: e.target.checked })
                  }
                }}
                disabled={cancelData.cancellationType === 'throughAdmin'}
                className="primary-check"
              />
              <span className="text-content-secondary text-sm">
                Send cancellation notification to studio via email
              </span>
            </label>
          </div>
