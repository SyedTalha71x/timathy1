...
import { Calendar, CalendarIcon, Edit, FileText, Info, MoreVertical, Plus, Trash2, Users, X, Pencil, ChevronDown, ChevronUp, Globe } from 'lucide-react'
...
import { LeadSpecialNoteIcon, extractNotes } from '../../admin-dashboard-components/shared/special-note/shared-special-note-icon'
...
const SortableLeadCard = ({
  ...
  const isInDemoColumn = columnId === "trial"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-white font-bold text-sm truncate block">
                {lead.studioName || `${lead.firstName} ${lead.surname}`}
              </span>
              {lead.studioName && (
      ...
      >
        <div className="flex items-center mb-2 sm:mb-3 relative">
          ...
          <div className="flex-1 mt-6">
            <h4 className="font-bold text-white text-lg mb-0.5">{lead.studioName || `${lead.firstName} ${lead.surname}`}</h4>
            {lead.studioName && (
              <p className="text-gray-300 text-sm mb-1">{lead.firstName} {lead.surname}</p>
            )}
    ...
    <>
      {isCompactCollapsed ? (
            ...
            {lead.websiteLink && (
              <div className="flex items-center gap-1 mt-1">
                ...
                <a 
                  ...
                  className="text-blue-400 text-xs hover:text-blue-300 truncate"
                  onClick={(e) => e.stopPropagation()}
                  onPointerDown={(e) => e.stopPropagation()}
                >
                  {lead.websiteLink.replace(/^https?:\/\//, '')}
                </a>
              </div>
            )}
        ...
        <div className="flex justify-center" onPointerDown={(e) => e.stopPropagation()}>
          {isInDemoColumn ? (
            // Demo Access column: show Create Contract button
            <button
              onClick={(e) => {
                e.stopPropagation()
