            ...
            {messages.length > 0 ? (
              <div className="grid gap-4 md:grid-cols-2">
                {messages.map((message) => (
                  <Card key={message.id} className={`!border-l-4 ${getBorderColor(message.type)}`}>
                    {message.image && (
                      <div className="relative mb-3 rounded-lg overflow-hidden border border-border h-40 -mt-1">
                        <img
                          src={message.image}
                          alt={message.title}
                    ...
                    <div className="flex justify-between items-start mb-2 gap-2">
                      <h3 className="text-content-primary font-semibold text-base flex-1">{message.title}</h3>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap ${getTagColor(message.type)}`}>
                        {message.type}
                      </span>
                    </div>
                    <p className="text-content-secondary text-xs sm:text-sm">{message.content}</p>
                  </Card>
                ))}
              </div>
            ) : (
