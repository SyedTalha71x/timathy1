...
import { fetchMyNutritionReminder, updateNutritionData } from "../../../features/notification/notificationSlice";

function NotificationsReminders() {
  const dispatch = useDispatch();
  const { nutritionReminders, loading } = useSelector((state) => state.reminder);

  useEffect(() => {
    dispatch(fetchMyNutritionReminder());
  }, [dispatch]);

  const toggleReminder = (key) => {
    if (!nutritionReminders) return;
    const updated = { ...nutritionReminders, [key]: !nutritionReminders[key] };
    dispatch(updateNutritionData(updated));
  };
  ...
  if (loading) return <p className="text-gray-400">Loading reminders...</p>;
            ...
            <div key={item.key} className="bg-[#212121] border border-[#383838] rounded-lg p-6 flex items-start justify-between gap-4">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-[#1a1a1a] ${
                  nutritionReminders?.[item.key] ? "bg-indigo-600" : "bg-gray-600"
                }`}
                role="switch"
                aria-checked={nutritionReminders?.[item.key]}
              >
                <span
                  className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                    nutritionReminders?.[item.key] ? "translate-x-5" : "translate-x-0"
                  }`}
                />
              </button>
            </div>
