      ...
      <div className="w-full">
        ...
        <div className="pt-6 sm:pt-8 pb-6 sm:pb-8 px-4 sm:px-8">
          ...
          <div className="relative w-full max-w-7xl mx-auto mb-6 sm:mb-8 px-2">
            <h2 className="text-lg sm:text-xl text-content-secondary text-center mb-4">
              How can we help you?
            </h2>
            ...
            {showSearchResults && filteredTopics.length > 0 && (
              <div className="absolute top-full left-2 right-2 mt-2 bg-surface-base border border-border rounded-xl shadow-lg z-50 max-h-64 overflow-y-auto">
                {filteredTopics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() => handleSelectTopic(topic)}
                    className="w-full text-left px-4 py-3 hover:bg-surface-hover border-b border-border last:border-b-0 transition-colors first:rounded-t-xl last:rounded-b-xl"
                  >
                    <p className="font-medium text-content-primary text-sm">{topic.title}</p>
                    <p className="text-content-muted text-xs mt-1">{topic.category}</p>
                  </button>
                ))}
              </div>
            )}
            ...
            {showSearchResults && searchQuery.trim() && filteredTopics.length === 0 && (
              <div className="absolute top-full left-2 right-2 mt-2 bg-surface-base border border-border rounded-xl shadow-lg z-50 p-4">
                <p className="text-content-muted text-sm text-center">No results found for "{searchQuery}"</p>
              </div>
            )}
          </div>
          ...
          <div className="w-full max-w-7xl mx-auto flex flex-col space-y-12">
            ...
            <div className="mb-12 sm:mb-16">
              <div className="flex items-center gap-2 mb-6 sm:mb-8">
                <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-content-primary">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {featuredArticles.map((article) => (
                  <button
                    ...
                    className="text-left bg-surface-card p-4 rounded-lg border border-border-subtle hover:bg-surface-hover hover:border-blue-500/50 transition-all group"
                  >
                    <div className="flex justify-between items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-content-primary group-hover:text-blue-400 transition-colors line-clamp-2 text-sm">
                          {article.title}
                        </h3>
                      ...
                      <ChevronRight className="w-4 h-4 text-content-faint flex-shrink-0 mt-0.5 group-hover:text-blue-400 transition-colors" />
                    </div>
                  </button>
                ))}
              </div>
            ...
            {/* Categories Section */}
            <div>
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-content-primary mb-6 sm:mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {filteredCategories.map((category) => (
                  <button
                    ...
                    className="text-left bg-surface-card p-6 sm:p-7 rounded-xl border border-border-subtle hover:bg-surface-hover hover:border-blue-500/50 transition-all group"
                  >
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-content-muted text-sm sm:text-base leading-relaxed">{category.description}</p>
                        {/* Article count */}
                        <p className="text-content-faint text-xs mt-3">{category.topics?.length || 0} articles</p>
                      </div>
                      <ChevronRight className="w-5 h-5 text-content-muted flex-shrink-0 mt-1 group-hover:text-blue-400 transition-colors" />
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="mt-12 pt-8 border-t border-border-subtle">
              <p className="text-center text-content-muted text-sm sm:text-base">
      ...
      <div className="flex-shrink-0 pt-8 sm:pt-12 lg:pt-16 pb-6 sm:pb-8 px-4 sm:px-8">
        <div className="w-full max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-2">
            <span className="text-lg text-content-secondary">Articles on:</span>
            {/* Orange Tag */}
            <span className="inline-block bg-primary text-white text-xs sm:text-sm font-bold px-3 py-1 rounded">
              {category.name}
            </span>
          </div>
          <p className="text-content-muted text-sm sm:text-base">{category.description}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col px-4 sm:px-8 pb-8">
        <div className="w-full max-w-7xl mx-auto grid lg:grid-cols-4 gap-6 sm:gap-8">
          ...
          <div className="lg:col-span-1">
            <h3 className="font-bold text-content-primary mb-4 text-sm sm:text-base">Categories</h3>
            ...
            <div className="space-y-3 sm:space-y-4">
              {category.topics.map((topic) => (
                <button
                  ...
                  className="w-full text-left bg-surface-card p-4 sm:p-5 rounded-xl border border-border-subtle hover:bg-surface-hover hover:border-blue-500/50 transition-all group"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-content-primary group-hover:text-blue-400 transition-colors text-sm sm:text-base">
                        {topic.title}
                      </h4>
                      <p className="text-content-muted text-xs sm:text-sm mt-2 line-clamp-2">{topic.content}</p>
                      {/* Reading time */}
                    ...
                    <ChevronRight
                      className={`w-5 h-5 text-content-muted flex-shrink-0 transition-transform group-hover:text-blue-400 ${
      ...
      <div className="flex-shrink-0 pt-8 sm:pt-12 lg:pt-16 pb-6 sm:pb-8 px-4 sm:px-8">
        <div className="w-full max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3">
            <span className="text-content-muted text-sm">Articles on:</span>
            {/* Orange Tag */}
            <span className="inline-block bg-primary text-white text-xs sm:text-sm font-bold px-3 py-1 rounded">
              {topic.category}
            </span>
      ...
      <div className="flex-1 flex flex-col px-4 sm:px-8 pb-8">
        <div className="w-full max-w-7xl mx-auto space-y-8">
          ...
          <div className="bg-surface-card rounded-xl p-6 sm:p-8 lg:p-10 border border-border-subtle">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-content-primary mb-4 sm:mb-6">
              {topic.title}
            </h1>

            <div className="flex flex-wrap items-center gap-4 mb-8 sm:mb-10 pb-8 sm:pb-10 border-b border-border-subtle text-sm text-content-muted">
            ...
            <div className="prose prose-sm sm:prose lg:prose-lg max-w-none">
              <p className="text-content-secondary leading-relaxed text-base sm:text-lg mb-6">
                {topic.content}
              </p>
              <p className="text-content-secondary leading-relaxed text-base sm:text-lg">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                        feedback === 'helpful'
                          ...
                          : 'bg-surface-hover text-content-secondary hover:bg-surface-button-hover border border-transparent'
                      }`}
                    >
                      <ThumbsUp size={16} />
                      Yes
                ...
                <div className="flex items-center gap-3">
                  {previousTopic && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2.5 bg-surface-hover hover:bg-surface-button-hover rounded-xl text-content-secondary hover:text-content-primary transition-colors text-sm"
                    >
                      <ArrowLeft size={16} />
                      Previous
                    </button>
          ...
          <div className="bg-gradient-to-r from-blue-500/20 to-blue-600/10 border border-blue-600/30 rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-content-primary font-semibold">Still need help?</h3>
                <p className="text-content-muted text-sm">Our support team is ready to assist you</p>
              </div>
            </div>
            <button 
